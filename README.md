...
Story2Script AI 是一个面向小说作者的 AI 剧本改编 Web 工作台。它把小说文本转换为可编辑、可校验、可预览、可导出的结构化 YAML 剧本初稿；系统应至少能处理 3 章以上小说，但不会把 1 章或 2 章小说拦在生成路径外。核心目标不是一次性生成一段不可控文本，而是把改编过程拆成可理解、可验证、可继续打磨的工作流。
...
- YAML 结构校验：AJV 校验 `docs/yaml-schema.md` v1.0 的必填字段、枚举、数组结构和非空章节数组。
- YAML 应用层校验：引用完整性、`dialogue`/`inner_voice` 条件 speaker、ID 唯一、章节覆盖 warning。
- OpenAI-compatible 单阶段生成：`LLM_PROVIDER=openai` 时可调用兼容 `/responses` 的模型服务，并返回即时校验结果。
- 章节切分：支持中文章节、英文 `Chapter`、Markdown 标题和 `---chapter---` 分隔符；1 章、2 章和更多章节都会返回有序章节数组。
...
- 输入预检：生成前会先识别章节结构；空输入会给出中文提示，1 章及以上会继续进入生成路径。
...
通过网盘分享的文件：6月7日.mp4
链接: https://pan.baidu.com/s/1pvCSTecy2E_vQkC4Rysjbw 提取码: u8cx
...
| LLM | MockProvider、OpenAI-compatible Responses API |
...
3. 点击 `用样例生成`。
4. 前端先调用 `/api/chapters/split` 识别章节结构；空输入会显示友好提示并停止生成。
5. 章节预检通过后，调用 `/api/screenplay/generate` 生成 YAML。
...
- 结构校验：由 `packages/shared/src/schema.ts` 的 JSON Schema 和 AJV 执行，覆盖必填字段、枚举、数组结构和非空章节数组。
...
- `OpenAIProvider`：读取 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`，调用 OpenAI-compatible `/responses`，从 Responses `output_text` / `output[].content[].text` 提取 YAML 或 JSON。
